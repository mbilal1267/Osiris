...
import { creators, campaigns, deals } from "@/data/seed";
...
export default function BrandDashboard() {
  ...
  const brandDeals = deals.filter((d) => d.brandId === "1");

  const createCampaign = () => {
    if (campaignName) { setShowModal(false); setCampaignName(""); setToast("Campaign created!"); setTimeout(() => setToast(""), 3000); }
  };

  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <div><h1 className="font-display text-3xl font-bold">Welcome back, {user?.name?.split(" ")[0] || "Brand"}</h1><p className="text-gray-500 mt-1">Manage your creator partnerships and campaigns.</p></div>
      ...
      <div className="grid lg:grid-cols-2 gap-6 mb-8">
        <div className="bg-white rounded-2xl border border-gray-100 p-6">
          <div className="flex items-center justify-between mb-4"><h2 className="font-bold text-lg">Active Campaigns</h2><Link href="/app/brand/campaigns" className="text-sm text-brand font-medium hover:underline">View all</Link></div>
          <div className="space-y-3">{brandCampaigns.slice(0, 3).map((c) => (
            <div key={c.id} className="flex items-center justify-between p-3 rounded-xl hover:bg-gray-50">
              <div><p className="font-medium text-sm">{c.name}</p><p className="text-xs text-gray-400">{c.timeline}</p></div>
              <div className="flex items-center gap-3"><TagPill label={c.status} type="status" /><span className="text-sm font-bold">{formatCurrency(c.budget)}</span></div>
            </div>
          ))}</div>
        </div>
        <div className="bg-white rounded-2xl border border-gray-100 p-6">
          <div className="flex items-center justify-between mb-4"><h2 className="font-bold text-lg">Suggested Creators</h2><Link href="/app/brand/creators" className="text-sm text-brand font-medium hover:underline">Discover more</Link></div>
          <div className="space-y-3">{creators.slice(0, 4).map((c) => (
            <Link key={c.id} href={`/c/${c.handle}`} className="flex items-center gap-3 p-3 rounded-xl hover:bg-gray-50">
              <div className="w-10 h-10 rounded-full bg-brand/10 flex items-center justify-center text-brand text-xs font-bold">{getInitials(c.name)}</div>
              <div className="flex-1 min-w-0"><p className="font-medium text-sm truncate">{c.name}</p><p className="text-xs text-gray-400">@{c.handle} · {formatNumber(c.followers)} followers</p></div>
              <div className="flex gap-1 flex-wrap">{c.niches.map((n) => <TagPill key={n} label={n} />)}</div>
            </Link>
          ))}</div>
        </div>
      </div>
